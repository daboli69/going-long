...
test('Defensive memory is usage-matched, lineage-backed and not promoted while uncertain',t=>{
 const {api:a}=setup(t),kickoff=future();a.BET.learning={completeness:{nfl:{status:'FRESH',actually_ingested:17,expected_completed:17},ncaa:{status:'FRESH',actually_ingested:115,expected_completed:115,supported_universe:'FBS regular-season games only'}},unsupported_granularity:['WR slot/outside'],defensive_weaknesses:[{id:'B|TE_RECEIVING',defense:'B',historical_weakness:'TE_RECEIVING',current_status:'uncertain',confidence:'low',historical:{season:2025,value:1.2},current:{season:2026,value:-.2,sample:8},weighted_estimate:.9,weights:{historical_weight:.8,current_weight:.2},opponent_adjustment:'same player comparison'}],matchup_signals:[{player_id:'p',player:'Player',team:'A',opponent:'B',kickoff,role:'TE_RECEIVING',status:'uncertain',confidence:'low',active_signal:false,usage:{opportunities:5,share:.2},weakness_id:'B|TE_RECEIVING',plain_language:'MATCHUP CONTEXT',lineage:['prediction','weakness','raw']}],coverage_matchup_signals:[{qb_id:'q',qb:'Quarterback',team:'A',opponent:'B',kickoff,role:'TE_RECEIVING',active_signal:false,projected_game_state:{period:'1H',defense_margin:3},coverage_tendency:{season:2025,shell:'COVER_3',rate:.42,plays:30,state_dropbacks:72},qb_tendency:{position:'TE',shell:'COVER_3',target_rate:.41,overall_target_rate:.25,targets:29,dropbacks:48},defense_role_weakness:{status:'uncertain'},coverage_role_results:{status:'observed',adjusted_extra_yards_per_target:.4,targets:28},projection_adjustment:'not promoted',lineage:['state','qb split','weakness']}]};
 ...
 const markup=a.defensiveMemoryMarkup();assert.match(markup,/0 active matchup edges/);assert.match(markup,/0 active coverage chains/);assert.match(markup,/COVER 3 42\.0%/);assert.match(markup,/Quarterback → TE: 41\.0%/);assert.match(markup,/UNCERTAIN — WATCH/);assert.match(markup,/FBS regular-season games only/);assert.match(markup,/prediction → weakness → raw/);
});
...
test('DFS opens the current GOING projection slate without requiring a salary CSV',t=>{
 ...
 a.BET.history={generated_at:'2026-09-09T10:00:00Z',profiles:{p:{id:'p',name:'Runner One',team:'A',position:'RB',last_game:'2026-09-01',stats:{rush_yds:model(65,18),rec_yds:model(28,12),receptions:model(3.5,1.4),rush_tds:model(.45,.3),rec_tds:model(.12,.2)}},q:{id:'q',name:'Receiver Two',team:'B',position:'WR',last_game:'2026-09-01',stats:{rush_yds:model(4,4),rec_yds:model(72,20),receptions:model(5.5,1.8),rush_tds:model(.03,.1),rec_tds:model(.4,.3)}}}};
 a.wireBetting();a.renderBetting();const panel=w.document.querySelector('#btDfsPanel');assert.equal(panel.hidden,false);assert.equal(w.document.querySelector('#dfsSource').value,'model');assert.equal(w.document.querySelector('#dfsCsvFields').hidden,true);assert.match(panel.textContent,/Runner One/);assert.match(panel.textContent,/Salary not loaded/);assert.match(panel.textContent,/no CSV/i);
 const addRunner=[...w.document.querySelectorAll('#dfsPool [data-dfs-toggle]')].find(button=>button.closest('.dfs-player').textContent.includes('Runner One'));addRunner.click();assert.match(w.document.querySelector('#dfsPicks').textContent,/Runner One/);assert.match(w.document.querySelector('#dfsSuggestions').textContent,/Receiver Two/);assert.doesNotMatch(w.document.querySelector('#dfsSuggestions').textContent,/Runner One/);
 const contest=w.document.querySelector('#dfsContest');contest.value='showdown';contest.dispatchEvent(new w.Event('change',{bubbles:true}));assert.equal(w.document.querySelector('#dfsCaptainSection').hidden,false);assert.match(w.document.querySelector('#dfsCaptainTitle').textContent,/Captain candidates · A @ B/);assert.match(w.document.querySelector('#dfsCaptains').textContent,/Runner One/);
 const captain=[...w.document.querySelectorAll('#dfsCaptains [data-dfs-captain]')].find(button=>button.closest('.dfs-captain').textContent.includes('Runner One'));captain.click();assert.match(w.document.querySelector('#dfsPicks').textContent,/Runner One.*Captain/s);assert.equal([...w.document.querySelectorAll('#dfsCaptains [data-dfs-captain]')].find(button=>button.closest('.dfs-captain').textContent.includes('Runner One')).getAttribute('aria-pressed'),'true');
});
...
test('GOING SCORE risks use the actual market evidence instead of one matchup fallback',t=>{
 ...
 const watch=a.goingScoreRisk({...base,market:'receptions',projection:6.5,projectionLabel:'6.5 rec',projectionSd:3,line:5.5,components:{...base.components,role:{label:'Role',percentile:70,evidence:'supported'},environment:{label:'Environment',percentile:60,evidence:'supported'},matchup:{label:'Conditional coverage watch',percentile:null,watch:true,evidence:'Cover 3 chain is real but current weakness is uncertain.'}}});assert.match(watch,/not yet promoted/);assert.match(watch,/Cover 3 chain/);
});
