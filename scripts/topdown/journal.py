...
class Journal:
    ...
    def sync(self):
        ...
        quote_filter='' if os.getenv('SUPABASE_SYNC_RAW_QUOTES')=='1' else " AND kind NOT IN ('quote','reference') AND (kind!='prediction' OR json_extract(payload,'$.actionable')=1 OR json_extract(payload,'$.tracking_group') IS NOT NULL)"
