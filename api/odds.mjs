...
export default async function handler(req,res){
  ...
  const saved=()=>{
    ...
    sendJson(req,res,{...hit.body,feed_status:'saved',feed_notice:'Provider refresh failed. Saved prices remain visible with their original timestamps; confirm current prices before betting.',retry_at:new Date(failures.get(sport)).toISOString()},200,60);
    return true;
  };
  const retry=failures.get(sport);
  if(retry&&Date.now()<retry){if(saved())return;return sendJson(req,res,{error:'Live provider unavailable; requests are paused briefly to avoid repeated failures.',provider:'parlay',retry_at:new Date(retry).toISOString()},503,60);}
  try{
