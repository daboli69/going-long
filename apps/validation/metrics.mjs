...
export function summarize(records,group='alerts'){
 ...
 return {predictions:byKind('prediction').filter(p=>group==='alerts'?!p.tracking_group:p.tracking_group===group),graded,bands:Object.entries(bands).map(([band,b])=>({band,...b,games:b.games.size,roi:b.profit/(100*b.bets),brier:b.decisive?b.error/b.decisive:null})),profit:performance.endingProfit,bets:graded.length,games:new Set(graded.map(p=>p.event)).size,...performance};
}
