export function summarize(records){
 ...
 const picks=byKind('prediction').filter(p=>p.actionable&&Date.parse(p.observed_at)<Date.parse(p.kickoff)).sort((a,b)=>Date.parse(a.observed_at)-Date.parse(b.observed_at)||b.ev-a.ev),seen=new Set(),graded=[],bands={};
