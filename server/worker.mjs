...
export async function oddsResponse(request,env,ctx={waitUntil(){}}){
  ...
  let cache;
  try{
    // Workers for Platforms disables caches.default; named caches are optional.
    cache=await globalThis.caches?.open('going-long-parlay-v1');
  ...
  try{
    ...
    if(cache)ctx.waitUntil(cache.put(cacheKey,Response.json(body,{headers:{'Cache-Control':`public,max-age=${TTL/1000}`}})).catch(()=>{}));
    return json(body);
  }catch(error){
...
export default {
  async fetch(request,env,ctx){
    ...
    if(url.pathname==='/api/snapshot'){
      ...
      if(!SNAPSHOTS.has(path))return json({error:'Unknown snapshot'},400);
      ...
      try{
        const r=await fetch('https://raw.githubusercontent.com/daboli69/going-long/main'+path,{signal:AbortSignal.timeout(10000)});
        if(r.ok){
          const body=await r.json();
          if(path!=='/data/history.json'||body.betting?.schema_version===1){
            const text=JSON.stringify(body);snapshots.set(path,{at:Date.now(),text});
